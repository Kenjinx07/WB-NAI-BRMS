<?php 
    ...
    if (isset($_GET['delete_id'])) {
      $B_ID = $_GET['delete_id'];
      $sql_delete = "DELETE FROM `blotters` WHERE `B_ID` = ?";
      $stmt = $conn->prepare($sql_delete);
      $stmt->bind_param('i', $B_ID);
      
      if ($stmt->execute()) {
          echo "<script>alert('Blotter successfully deleted.');</script>";
      } else {
          echo "<script>alert('Failed to remove the blotter case.');</script>";
      }
      $stmt->close();
  }
?>
...
<head>
  ...
  <script>
    function confirmDeletionB(B_ID) {
        if (confirm('Are you sure you want to delete this blotter case?')) {
            window.location.href = 'view_blotter.php?delete_id=' + B_ID;
        }
    }
</script>
</head>
<body>
  ...
  <div class="main">
    <div class="main-top">
      <h1>Barangay Blotters</h1>
    </div>

    <?php 
    function viewblotter($conn, $sql) {
        $row = "";
        ...
        if($query = mysqli_query($conn, $sql)) {
            if(mysqli_num_rows($query) > 0){
                while($result = mysqli_fetch_array($query)){
                    $B_ID = $result['B_ID'];
                    $BF_Name = $result['BF_Name'];
                    $BM_Name = $result['BM_Name'];
                    $BL_Name = $result['BL_Name'];
                    $B_Email = $result['B_Email'];
                    $B_Type = $result['B_Type'];
                    $B_Description = $result['B_Description'];
                    ...
                    $row .= "<tr>
                        ...
                        <td><a onclick='confirmDeletionB($B_ID)'>Delete</a></td>
                        </tr>";
                }
